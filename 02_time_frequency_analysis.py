...
except Exception:  # fallback handled in helper
    fdrcorrection = None
...
SIG_MASK_PARAMS_DEFAULT = dict(config.get("time_frequency_analysis.sig_mask_params", {
    "marker": "o",
...
ROI_HIGHLIGHT_ENABLED = bool(config.get("time_frequency_analysis.roi_highlight_enabled", False))
ANNOTATE_DESCRIPTIVE_TOPO = bool(config.get("time_frequency_analysis.annotate_descriptive_topo", True))
SIG_ALPHA = float(config.get("statistics.sig_alpha", 0.05))
CLUSTER_N_PERM = int(config.get("statistics.cluster_n_perm", 1024))
CLUSTER_REPORT_SIZE = bool(config.get("statistics.cluster_report_size", False))
CLUSTER_REPORT_MASS = bool(config.get("statistics.cluster_report_mass", False))
DIFF_ANNOTATION_ENABLED = bool(config.get("statistics.diff_annotation_enabled", False))
...
# Allow misaligned trim flag (new location under 'alignment', with backward-compatible fallback)
ALLOW_MISALIGNED_TRIM = bool(
    config.get("alignment.allow_misaligned_trim",
...
def _log_baseline_qc(
    tfr_obj: "mne.time_frequency.EpochsTFR",
    baseline: Tuple[Optional[float], Optional[float]],
    logger: Optional[logging.Logger] = None,
) -> None:
    """Compute simple baseline QC metrics and log a summary.

    Reports number of baseline time points, median temporal std across channels/freqs,
    epoch-wise robust CV (1.4826*MAD/|median|). Warns if baseline window is empty
    or QC metrics are NaN.
    """
    try:
        times = np.asarray(tfr_obj.times)
        b_start, b_end, idx = _validate_baseline_indices(times, baseline)
    except Exception as e:
        msg = f"Baseline QC: invalid baseline window ({baseline}): {e}"
        if logger:
            logger.warning(msg)
        else:
            print(f"Warning: {msg}")
    ...
    try:
        ...
        # temporal std per (epoch, ch, freq)
        temporal_std = np.nanstd(base, axis=-1)
        median_temporal_std = float(np.nanmedian(temporal_std))
        # epoch mean across (ch, freq, time)
        ...
        n_time = int(len(idx))
        msg = (
            f"Baseline QC: n_time={n_time}, median_temporal_std={median_temporal_std:.3g}, "
            f"epoch_MAD={mad:.3g}, RCV={rcv:.3g}"
        )
        log = logger.info if logger else print
        log(msg)
        if not np.isfinite(median_temporal_std) or not np.isfinite(rcv):
            warn = "Baseline QC: non-finite metrics detected; baseline may be unstable."
            (logger.warning if logger else print)(warn)
    except Exception as e:
        msg = f"Baseline QC failed: {e}"
        if logger:
            logger.warning(msg)
        else:
            print(f"Warning: {msg}")
...
## _canonicalize_ch_name moved to roi_utils


## _find_roi_channels moved to roi_utils



## _build_rois moved to roi_utils
...
def _setup_logging(subject: str) -> logging.Logger:
    ...
    return get_subject_logger("time_frequency_analysis", subject, LOG_FILE_NAME)


def _setup_group_logging() -> logging.Logger:
    """Backward-compatible wrapper using centralized logging utils."""
    return get_group_logger("time_frequency_analysis", LOG_FILE_NAME)
...
def _save_fig(fig_obj: Any, out_dir: Path, name: str, formats: Optional[list[str]] = None, logger: Optional[logging.Logger] = None) -> None:
    ...
    for i, f in enumerate(figs):
        # Add a small footer annotation with units and baseline for transparency
        try:
            bwin = config.get("time_frequency_analysis.baseline_window", [-5.0, -0.01])
            foot = f"Units: log10(power/baseline) | Baseline: [{float(bwin[0]):.2f}, {float(bwin[1]):.2f}] s"
            try:
                # Avoid stacking repeated footers if _save_fig called on the same figure
                f.text(0.01, 0.01, foot, fontsize=8, alpha=0.8)
            except Exception:
                pass
        except Exception:
            pass
...
def _plot_topomap_on_ax(
    ...
    # Optionally annotate figure that topomaps are descriptive unless explicitly masked by stats
    try:
        if ANNOTATE_DESCRIPTIVE_TOPO and hasattr(ax, 'figure'):
            fig = ax.figure
            # Avoid duplicating the note on multi-axes figures
...
def _get_consistent_bands(max_freq_available: Optional[float] = None) -> Dict[str, Tuple[float, float]]:
    ...
    # Include delta and theta if defined in configuration (low-frequency bands first)
    if "delta" in BAND_BOUNDS:
        bands["delta"] = BAND_BOUNDS["delta"]
...
def _format_t_label(t: float, df: int, p: float) -> str:
    try:
        if np.isfinite(t) and isinstance(df, (int, np.integer)):
            return f"t({df})={t:.2f}, {_format_p_value(float(p))}"
        return f"t=nan, {_format_p_value(float(p))}"
    except Exception:
        return f"t=nan, p=nan"


def _paired_t_text(a: np.ndarray, b: np.ndarray) -> str:
    """Return a concise paired t-test label for arrays a and b.

    - Flattens inputs and removes NaNs pairwise
    - Uses scipy.stats.ttest_rel
    - Returns label like: "t(df)=x.xx, p=.yyy"
    """
    try:
        ...
        if a2.size < 2 or b2.size < 2:
            return "t(n<2)"
        res = ttest_rel(a2, b2, nan_policy="omit")
        df = int(a2.size - 1)
        t_val = float(res.statistic) if np.isfinite(res.statistic) else np.nan
        p_val = float(res.pvalue) if np.isfinite(res.pvalue) else np.nan
        return _format_t_label(t_val, df, p_val)
    except Exception:
        return "t=err"


def _format_cluster_ann(p: Optional[float], k: Optional[int] = None, mass: Optional[float] = None) -> str:
    try:
        parts = []
        if p is not None and np.isfinite(p):
            parts.append(_format_p_value(float(p)))
        if CLUSTER_REPORT_SIZE and k is not None and isinstance(k, (int, np.integer)) and k > 0:
            parts.append(f"k={int(k)}")
        if CLUSTER_REPORT_MASS and mass is not None and np.isfinite(mass):
...
def _extract_epochwise_channel_values(
    ...
    """Return per-epoch, per-channel mean over the given band/time window.

    Shape: (n_epochs, n_channels). Returns None if selection invalid.
    Assumes `tfr_epochs` is baseline-corrected in logratio units.
    """
    try:
        ...
        if fmask.sum() == 0 or tmask.sum() == 0:
            return None
        # data: (n_epochs, n_channels, n_freqs, n_times)
...
def _get_eeg_adjacency(info: mne.Info, restrict_picks: Optional[np.ndarray] = None) -> Tuple[Optional[np.ndarray], Optional[np.ndarray], Optional[mne.Info]]:
    """Return (adjacency, eeg_picks, info_eeg) for EEG channels in `info`.

    If adjacency cannot be computed, returns (None, eeg_picks, info_eeg) so that
    callers can still fall back to per-channel tests.
    """
    try:
...
def _build_full_mask_from_eeg(sig_mask_eeg: np.ndarray, n_ch_total: int, eeg_picks: np.ndarray) -> np.ndarray:
...
def _cluster_mask_from_clusters(
    clusters: list,
    p_vals: np.ndarray,
    n_features: int,
    alpha: float,
) -> np.ndarray:
    """Combine significant clusters into a boolean mask over features.

    Supports clusters provided as index arrays or boolean masks.
    """
...
def _cluster_mask_two_sample_arrays(
    Xa: np.ndarray,
    Xb: np.ndarray,
    info: mne.Info,
    alpha: float = SIG_ALPHA,
    paired: bool = False,
    n_permutations: int = CLUSTER_N_PERM,
    restrict_picks: Optional[np.ndarray] = None,
) -> Optional[np.ndarray]:
    """Compute EEG sensor-level cluster mask for two-sample comparison.

    Xa, Xb: arrays of shape (n_samples, n_channels_total). Only EEG channels are used.
    Returns a boolean mask of length n_channels_total, with True for significant sensors
    belonging to any cluster with p <= alpha. Returns None on failure.
    """
    try:
        adjacency, eeg_picks, info_eeg = _get_eeg_adjacency(info, restrict_picks=restrict_picks)
        ...
        if paired and A.shape[0] == B.shape[0]:
            # 1-sample cluster test on paired differences
            Xd = A - B
            ...
            T_obs, clusters, p_values, _ = out
        else:
            # Two-sample independent cluster test
            ...
            T_obs, clusters, p_values, _ = out
        sig_eeg = _cluster_mask_from_clusters(clusters, p_values, n_features=A.shape[1], alpha=alpha)
        sig_full = _build_full_mask_from_eeg(sig_eeg, n_ch_total=len(info["ch_names"]), eeg_picks=eeg_picks)
        return sig_full
    except Exception:
        return None


def _cluster_test_two_sample_arrays(
    Xa: np.ndarray,
    Xb: np.ndarray,
    info: mne.Info,
    alpha: float = SIG_ALPHA,
    ...
    """Two-sample sensor-level cluster test; return (mask_full, p_min).

    - Xa, Xb: shape (n_samples, n_channels_total). EEG channels are used.
    - Returns a boolean mask for significant sensors and the minimum cluster p-value.
    ...
    try:
        ...
        sig_eeg = _cluster_mask_from_clusters(clusters, p_values, n_features=A.shape[1], alpha=alpha)
        sig_full = _build_full_mask_from_eeg(sig_eeg, n_ch_total=len(info["ch_names"]), eeg_picks=eeg_picks)
        # Choose p-value, size, and mass of the largest cluster by |T| sum
        ...
        try:
            if len(clusters) > 0:
                ...
                if len(masses) > 0:
                    ...
                    # derive indices for k-th cluster
                    cl_k = clusters[k]
...
def _fdr_bh_mask(p_vals: np.ndarray, alpha: float = SIG_ALPHA) -> Optional[np.ndarray]:
    try:
        p_vals = np.asarray(p_vals)
        if p_vals.ndim != 1 or p_vals.size == 0:
            return None
        if fdrcorrection is None:
            return (p_vals < float(alpha)).astype(bool)
        rej, _ = fdrcorrection(p_vals, alpha=float(alpha))
        return rej.astype(bool)
    except Exception:
        try:
            return (np.asarray(p_vals) < float(alpha)).astype(bool)
        except Exception:
            return None


def _fdr_bh_values(p_vals: np.ndarray, alpha: float = SIG_ALPHA) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
    """Return (reject, qvals) for BH-FDR; None on failure.

    If statsmodels is unavailable, uses p as q (conservative) and reject = p<alpha.
    """
    try:
        p_vals = np.asarray(p_vals)
        if p_vals.ndim != 1 or p_vals.size == 0:
            return None, None
        if fdrcorrection is None:
            rej = p_vals < float(alpha)
            q = p_vals.astype(float)
            return rej.astype(bool), q
        rej, q = fdrcorrection(p_vals, alpha=float(alpha))
        return rej.astype(bool), q.astype(float)
    except Exception:
        try:
            p_vals = np.asarray(p_vals)
            rej = p_vals < float(alpha)
...
def _format_fdr_ann(q_min: Optional[float], k_rej: Optional[int]) -> str:
    try:
        parts = []
        if q_min is not None and np.isfinite(q_min):
            # format like p for consistency
            parts.append(f"FDR q={q_min:.3f}" if q_min >= 1e-3 else "FDR q<.001")
        if k_rej is not None and isinstance(k_rej, (int, np.integer)):
...
def _cluster_test_epochs(
    ...
    """Cluster test for epochwise values in a band/time window; return (mask, p_min)."""
    X = _extract_epochwise_channel_values(tfr_epochs, fmin=fmin, fmax=fmax, tmin=tmin, tmax=tmax)
    if X is None:
        return None, None
    try:
        ...
        if Xa.shape[0] < 2 or Xb.shape[0] < 2:
            return None, None
        return _cluster_test_two_sample_arrays(
...
def _cluster_mask_epochs(
    ...
    """Cluster-based permutation mask across EEG sensors using epochwise values.

    Returns boolean mask length = n_channels_total in `tfr_epochs.info`, or None if invalid.
    """
    X = _extract_epochwise_channel_values(tfr_epochs, fmin=fmin, fmax=fmax, tmin=tmin, tmax=tmax)
    if X is None:
        return None
    try:
        ...
        if Xa.shape[0] < 2 or Xb.shape[0] < 2:
            return None
        return _cluster_mask_two_sample_arrays(Xa, Xb, tfr_epochs.info, alpha=alpha, paired=paired, n_permutations=n_permutations)
    except Exception:
        return None
    ...
    for c, (band, (fmin, fmax)) in enumerate(bands.items()):
        ...
        if fmin >= fmax_eff:
            for r in range(n_rows):
                axes[r, c].axis('off')
            continue
        # Average data for each condition in this band
        ...
        try:
            mne.viz.plot_topomap(
                ...
                cmap=TOPO_CMAP,
                mask=(sig_mask if DIFF_ANNOTATION_ENABLED else None),
                mask_params=SIG_MASK_PARAMS_DEFAULT,
            )
        except Exception:
            _plot_topomap_on_ax(
                ax,
    ...
    for r, (band, (fmin, fmax)) in enumerate(bands.items()):
        ...
        if max_data is None or min_data is None:
            for c in range(n_cols):
                axes[r, c].axis("off")
            continue
        # Cluster/FDR significance (guarded by config)
        sig_mask = None
        cluster_p_min = cluster_k = cluster_mass = None
        fdr_txt = ""
        if DIFF_ANNOTATION_ENABLED:
        ...
        try:
            cl_txt = (_format_cluster_ann(cluster_p_min, cluster_k, cluster_mass) or fdr_txt) if DIFF_ANNOTATION_ENABLED else ""
            if cl_txt:
                ax.text(0.5, 1.02, f"Δ%={pct_mu:+.1f}% | {cl_txt}", transform=ax.transAxes, ha="center", va="top", fontsize=9)
            else:
...
def contrast_pain_nonpain_topomaps_rois(
    ...
    for roi, roi_chs in roi_map.items():
        ...
        # New layout: rows=Pain/Non/Diff, columns=bands
        n_rows = 3
        n_cols = len(bands)
        fig, axes = plt.subplots(
            n_rows,
        ...
        for c, (band, (fmin, fmax)) in enumerate(bands.items()):
            ...
            diff_abs = _robust_sym_vlim(diff_data) if np.isfinite(diff_data).any() else 0.0
            # ROI-mean annotation values
            if mask_vec.any():
                pain_mu = float(np.nanmean(pain_data[mask_vec])); non_mu = float(np.nanmean(non_data[mask_vec])); diff_mu = float(np.nanmean(diff_data[mask_vec]))
            else:
                pain_mu = float(np.nanmean(pain_data)); non_mu = float(np.nanmean(non_data)); diff_mu = float(np.nanmean(diff_data))
            # Row 0: Pain
            ax = axes[0, c]
            try:
                mne.viz.plot_topomap(
                    pain_data, tfr_pain.info, axes=ax, show=False,
                    vlim=(vmin, vmax) if vmin is not None and vmax is not None else None,
                    mask=(mask_vec if ROI_HIGHLIGHT_ENABLED else None), mask_params=mask_params,
                    cmap=TOPO_CMAP,
                )
            except Exception:
                _plot_topomap_on_ax(
                    ax, pain_data, tfr_pain.info,
                    mask=(mask_vec if ROI_HIGHLIGHT_ENABLED else None), mask_params=mask_params,
                    vmin=vmin, vmax=vmax,
                )
            try:
            ...
            try:
                mne.viz.plot_topomap(
                    non_data, tfr_pain.info, axes=ax, show=False,
                    vlim=(vmin, vmax) if vmin is not None and vmax is not None else None,
                    mask=(mask_vec if ROI_HIGHLIGHT_ENABLED else None), mask_params=mask_params,
                    cmap=TOPO_CMAP,
                )
            ...
            sig_mask = None; cluster_p_min = cluster_k = cluster_mass = None; fdr_txt = ""
            if DIFF_ANNOTATION_ENABLED:
                sig_mask, cluster_p_min, cluster_k, cluster_mass = _cluster_test_epochs(
                    tfr_sub, pain_mask, non_mask, fmin=fmin, fmax=fmax_eff, tmin=tmin, tmax=tmax, paired=False,
            ...
            try:
                mne.viz.plot_topomap(
                    diff_data, tfr_pain.info, axes=ax, show=False,
                    vlim=((-diff_abs, +diff_abs) if diff_abs > 0 else None),
                    mask=(mask_vec if ROI_HIGHLIGHT_ENABLED else None), mask_params=mask_params,
                    cmap=TOPO_CMAP,
                )
                ...
                _plot_topomap_on_ax(
                    ax, diff_data, tfr_pain.info,
                    mask=(mask_vec if ROI_HIGHLIGHT_ENABLED else None), mask_params=mask_params,
                    vmin=(-diff_abs if diff_abs > 0 else None), vmax=(+diff_abs if diff_abs > 0 else None),
                )
            # Overlay significance markers if enabled
            if DIFF_ANNOTATION_ENABLED and sig_mask is not None and sig_mask.any():
                try:
                    mne.viz.plot_topomap(
                        diff_data, tfr_pain.info, axes=ax, show=False,
                        vlim=((-diff_abs, +diff_abs) if diff_abs > 0 else None), cmap=TOPO_CMAP,
                        mask=sig_mask, mask_params=SIG_MASK_PARAMS_DEFAULT,
                    )
                except Exception:
                    try:
                        _plot_topomap_on_ax(
...
def _align_paired_avg_tfrs(
    ...
    """Align paired AverageTFR lists across subjects to common channels/freqs/times.

    Returns (info_common, data_a, data_b) with shapes:
        - data_a: (n_subjects, n_channels, n_freqs, n_times)
        - data_b: (n_subjects, n_channels, n_freqs, n_times)

    Only retains subjects for which both A and B match the base times/freqs and
    for which there is at least one common EEG channel across all retained pairs.
    """
    ...
    if not keep:
        return None, None, None
    # Compute common channel set across all pairs (intersection of A∩B per pair, then across pairs)
    per_pair_common = []
    for a, b in keep:
    ...
    if len(common) == 0:
        ...
        return None, None, None
    # Stack aligned data in common channel order
    data_a = []
    data_b = []
    for a, b in keep:
...
def group_topomaps_bands_all_trials(
    ...
    tmin = float(max(times.min(), tmin_req))
    tmax = float(min(times.max(), tmax_req))
    # Horizontal layout: 1 row x n_cols (bands)
    n_cols = len(bands)
    fig, axes = plt.subplots(1, n_cols, figsize=(4.8 * n_cols, 4.8))
    if n_cols == 1:
        axes = [axes]
    # Reduce whitespace between columns similar to other horizontal grids
    try:
    ...
    for c, (band, (fmin, fmax)) in enumerate(bands.items()):
        ...
        try:
            ...
            _plot_topomap_on_ax(ax, vec, info_common, vmin=-vabs, vmax=+vabs)
        ...
        try:
            ...
            pass
        # Put band label as title above each map
        try:
...
def group_topomaps_pain_nonpain(
    ...
    if len(avg_pain) == 0 or len(avg_non) == 0:
        ...
        return
    # Align paired across subjects to ensure pairing for t-tests
    info_p, data_p, data_n = _align_paired_avg_tfrs(avg_pain, avg_non, logger=logger)
    if info_p is None or data_p is None or data_n is None:
        if logger:
            logger.warning("Group pain/non-pain: could not align paired data")
        return
    ...
    for c, (band, (fmin, fmax)) in enumerate(bands.items()):
        ...
        # Row 0: Pain
        try:
            mne.viz.plot_topomap(v_p, info_p, axes=axes[0, c], show=False, vlim=(-vabs_pn, +vabs_pn), cmap=TOPO_CMAP)
        except Exception:
        ...
        # Row 1: Non
        try:
            mne.viz.plot_topomap(v_n, info_p, axes=axes[1, c], show=False, vlim=(-vabs_pn, +vabs_pn), cmap=TOPO_CMAP)
        except Exception:
        ...
        try:
            ...
            pass
        # Row 2: Diff with significance gating
        sig_mask = None; cluster_p_min = cluster_k = cluster_mass = None; fdr_txt = ""
        try:
            ...
            if DIFF_ANNOTATION_ENABLED:
                sig_mask, cluster_p_min, cluster_k, cluster_mass = _cluster_test_two_sample_arrays(
                    subj_p, subj_n, info_p, alpha=SIG_ALPHA, paired=True, n_permutations=CLUSTER_N_PERM
                )
                if sig_mask is None:
                    try:
                        res = ttest_rel(subj_p, subj_n, axis=0, nan_policy="omit")
                        p_ch = np.asarray(res.pvalue)
                        sig_mask = _fdr_bh_mask(p_ch, alpha=SIG_ALPHA)
                        rej, q = _fdr_bh_values(p_ch, alpha=SIG_ALPHA)
                        if q is not None:
                            q_min = float(np.nanmin(q))
                            k_rej = int(np.nansum(rej)) if rej is not None else int(np.sum(p_ch < SIG_ALPHA))
                            fdr_txt = _format_fdr_ann(q_min, k_rej)
                    except Exception:
                        sig_mask = None
        except Exception:
            sig_mask = None
        try:
            mne.viz.plot_topomap(
                v_d, info_p, axes=axes[2, c], show=False, vlim=(-vabs_d, +vabs_d), cmap=TOPO_CMAP,
                mask=(sig_mask if DIFF_ANNOTATION_ENABLED else None), mask_params=SIG_MASK_PARAMS_DEFAULT,
            )
        except Exception:
            _plot_topomap_on_ax(
                axes[2, c], v_d, info_p, vmin=-vabs_d, vmax=+vabs_d,
                mask=(sig_mask if DIFF_ANNOTATION_ENABLED else None), mask_params=SIG_MASK_PARAMS_DEFAULT,
            )
        try:
            mu_d = float(np.nanmean(v_d)); pct_d = (10.0 ** (mu_d) - 1.0) * 100.0
            cl_txt = (_format_cluster_ann(cluster_p_min, cluster_k, cluster_mass) or fdr_txt) if DIFF_ANNOTATION_ENABLED else ""
        ...
        try:
            ...
            sm_d.set_array([])
            fig.colorbar(sm_d, ax=axes[2, c], fraction=COLORBAR_FRACTION, pad=COLORBAR_PAD)
        except Exception:
            pass
    # Row labels and save
    try:
...
def group_pain_nonpain_temporal_topomaps(
    ...
    if info_p is None or data_p is None or data_n is None:
        if logger:
            logger.warning("Group temporal topomaps: could not align paired pain/non data")
        return
    ...
    for band_name, (fmin, fmax) in bands.items():
        ...
        for col, (w_start, w_end) in enumerate(zip(win_starts, win_ends)):
            ...
            if v_n is not None:
                try:
                    mne.viz.plot_topomap(v_n, info_p, axes=axes[1, col], show=False, vlim=(-vabs_cond, +vabs_cond), cmap=TOPO_CMAP)
                except Exception:
            ...
            if v_d is not None:
                ...
                fdr_txt = ""
                # Compute significance only if annotations are enabled
                if DIFF_ANNOTATION_ENABLED:
                    try:
                    ...
                    if sig_mask is None:
                        # FDR fallback across sensors using paired t across subjects
                        try:
                            res = ttest_rel(subj_p, subj_n, axis=0, nan_policy="omit")
                            p_ch = np.asarray(res.pvalue)
                            sig_mask = _fdr_bh_mask(p_ch, alpha=SIG_ALPHA)
                ...
                try:
                    ...
                    pct = (10.0 ** (mu) - 1.0) * 100.0
                    # Cluster/FDR annotation only when enabled
                    cl_txt = (_format_cluster_ann(p_min_window, k_win, mass_win) or fdr_txt) if DIFF_ANNOTATION_ENABLED else ""
                    if cl_txt:
...
def group_maxmin_temperature_temporal_topomaps(
    ...
    """Group temporal topomaps: max vs min temperature across multiple windows and bands.

    - Finds global min and max temperature levels across subjects (rounded to 0.1°C)
    - Builds paired per-subject AverageTFRs and aligns them
    - Splits the plateau into `window_count` equal windows
    - Rows: Max temp, Min temp, Difference; Columns: windows; separate figure per band
    """
    if not powers:
        return
    ...
    if len(temps) < 2:
        if logger:
            logger.warning("Group temporal (max/min): fewer than 2 temperature levels; skipping")
        return
    t_min = float(min(temps)); t_max = float(max(temps))

    # Build per-subject AverageTFR pairs for min and max temps (paired across same subjects)
    ...
    if len(avg_min) < 2 or len(avg_max) < 2:
        if logger:
            logger.warning("Group temporal (max/min): insufficient subjects with both min and max trials")
        return
    info_c, data_min, data_max = _align_paired_avg_tfrs(avg_min, avg_max, logger=logger)
    if info_c is None or data_min is None or data_max is None:
        if logger:
            logger.warning("Group temporal (max/min): could not align paired min/max data")
        return
    ...
    if not np.isfinite(tmin) or not np.isfinite(tmax) or tmax <= tmin:
        if logger:
            logger.warning("Group temporal (max/min): invalid plateau window after clipping")
        return
    edges = np.linspace(tmin, tmax, int(window_count) + 1)
    win_starts = edges[:-1]
    win_ends = edges[1:]
    ...
    for band_name, (fmin, fmax) in bands.items():
        ...
        for w_start, w_end in zip(win_starts, win_ends):
            tmask = (times >= w_start) & (times < w_end)
            if fmask.sum() == 0 or tmask.sum() == 0:
                max_vecs.append(None); min_vecs.append(None); diff_vecs.append(None)
                continue
        ...
        for col, (w_start, w_end) in enumerate(zip(win_starts, win_ends)):
            ...
            if v_max is not None:
                try:
                    mne.viz.plot_topomap(v_max, info_c, axes=axes[0, col], show=False, vlim=(-vabs_cond, +vabs_cond), cmap=TOPO_CMAP)
                except Exception:
            ...
            if v_min is not None:
                try:
                    mne.viz.plot_topomap(v_min, info_c, axes=axes[1, col], show=False, vlim=(-vabs_cond, +vabs_cond), cmap=TOPO_CMAP)
                except Exception:
            ...
            if v_diff is not None:
                ...
                if DIFF_ANNOTATION_ENABLED:
                    try:
                        ...
                        sig_mask, p_min_window, k_win, mass_win = _cluster_test_two_sample_arrays(
                            subj_max, subj_min, info_c, alpha=SIG_ALPHA, paired=True, n_permutations=CLUSTER_N_PERM
                        )
                    except Exception:
                        sig_mask, p_min_window, k_win, mass_win = None, None, None, None
                    if sig_mask is None:
...
def group_contrast_maxmin_temperature(
    ...
    info_common, data_min, data_max = _align_paired_avg_tfrs(avg_min, avg_max, logger=logger)
    if info_common is None or data_min is None or data_max is None:
        if logger:
            logger.info("Group max/min: could not align paired min/max TFRs; skipping")
        return
    ...
    # New layout: rows are Max/Min/Diff; columns are bands
    n_rows = 3
    n_cols = len(bands)
    ...
    for c, (band, (fmin, fmax)) in enumerate(bands.items()):
        ...
        ax = axes[0, c]
        try:
            mne.viz.plot_topomap(v_max, info_common, axes=ax, show=False, vlim=(-vabs_pn, +vabs_pn), cmap=TOPO_CMAP)
        except Exception:
            _plot_topomap_on_ax(ax, v_max, info_common, vmin=-vabs_pn, vmax=+vabs_pn)
        try:
            mu_max = float(np.nanmean(v_max)); pct_max = (10.0 ** (mu_max) - 1.0) * 100.0
        ...
        ax = axes[1, c]
        try:
            mne.viz.plot_topomap(v_min, info_common, axes=ax, show=False, vlim=(-vabs_pn, +vabs_pn), cmap=TOPO_CMAP)
        except Exception:
            _plot_topomap_on_ax(ax, v_min, info_common, vmin=-vabs_pn, vmax=+vabs_pn)
        try:
            mu_min = float(np.nanmean(v_min)); pct_min = (10.0 ** (mu_min) - 1.0) * 100.0
        ...
        if DIFF_ANNOTATION_ENABLED:
            try:
                ...
                sig_mask, cluster_p_min, cluster_k, cluster_mass = _cluster_test_two_sample_arrays(
                    subj_max, subj_min, info_common, alpha=SIG_ALPHA, paired=True, n_permutations=CLUSTER_N_PERM
                )
            except Exception:
                sig_mask, cluster_p_min, cluster_k, cluster_mass = None, None, None, None
            if sig_mask is None:
                try:
                    res = ttest_rel(subj_max, subj_min, axis=0, nan_policy="omit")
                    p_ch = np.asarray(res.pvalue)
        ...
        try:
            mne.viz.plot_topomap(
                v_diff, info_common, axes=ax, show=False,
                vlim=(-vabs_diff, +vabs_diff), cmap=TOPO_CMAP,
                mask=(sig_mask if DIFF_ANNOTATION_ENABLED else None), mask_params=SIG_MASK_PARAMS_DEFAULT
            )
        except Exception:
            _plot_topomap_on_ax(
                ax, v_diff, info_common, vmin=-vabs_diff, vmax=+vabs_diff,
                mask=(sig_mask if DIFF_ANNOTATION_ENABLED else None), mask_params=SIG_MASK_PARAMS_DEFAULT
            )
        try:
            mu_d = float(np.nanmean(v_diff)); pct_d = (10.0 ** (mu_d) - 1.0) * 100.0
            cl_txt = (_format_cluster_ann(cluster_p_min, cluster_k, cluster_mass) or fdr_txt) if DIFF_ANNOTATION_ENABLED else ""
            label = f"Δ%={pct_d:+.1f}%" + (f" | {cl_txt}" if cl_txt else "")
            ax.text(0.5, 1.02, label, transform=ax.transAxes, ha="center", va="top", fontsize=9)
        except Exception:
...
def group_contrast_pain_nonpain_scalpmean(
    ...
    """Group-level All-EEG (scalp-mean) TFRs for pain vs non-pain and their difference.

    Creates three figures (PNG+SVG):
    - group_tfr_AllEEG_pain_baseline_logratio
    - group_tfr_AllEEG_nonpain_baseline_logratio
    - group_tfr_AllEEG_pain_minus_non_baseline_logratio
    """
    if not powers:
        return
    avg_pain: List["mne.time_frequency.AverageTFR"] = []
    ...
    if len(avg_pain) < 1 or len(avg_non) < 1:
        return
    # Align across subjects (paired) and compute group means
    info_p, data_p, data_n = _align_paired_avg_tfrs(avg_pain, avg_non, logger=logger)
    if info_p is None or data_p is None or data_n is None:
        return
    ...
    mean_p = data_p.mean(axis=0)  # (n_ch, n_freqs, n_times)
    mean_n = data_n.mean(axis=0)
    # Scalp-mean collapse to 1 channel
    try:
        data_p_sm = np.asarray(mean_p).mean(axis=0, keepdims=True)  # (1, n_freqs, n_times)
        data_n_sm = np.asarray(mean_n).mean(axis=0, keepdims=True)
        diff_sm = data_p_sm - data_n_sm
    except Exception:
        return
    # Build AverageTFR clones for plotting
    try:
        # Use avg_pain[0] as a template
        ...
        # Pain
        grp_p = tmpl.copy()
        grp_p.data = data_p_sm
        grp_p.info = mne.create_info(["AllEEG"], sfreq=sfreq, ch_types='eeg')
        grp_p.nave = n_subj
        grp_p.comment = "Group AllEEG Pain"
        # Non
        grp_n = tmpl.copy()
        grp_n.data = data_n_sm
        grp_n.info = mne.create_info(["AllEEG"], sfreq=sfreq, ch_types='eeg')
        grp_n.nave = n_subj
        grp_n.comment = "Group AllEEG Non"
        # Diff
        grp_d = tmpl.copy()
        grp_d.data = diff_sm
        grp_d.info = mne.create_info(["AllEEG"], sfreq=sfreq, ch_types='eeg')
        grp_d.nave = n_subj
        grp_d.comment = "Group AllEEG Diff"

        # Plot and save
        fig = grp_p.plot(picks="AllEEG", show=False)
        try:
            fig.suptitle(f"Group TFR: All EEG — Pain (baseline logratio, n={n_subj})", fontsize=12)
        except Exception:
            pass
        _save_fig(fig, out_dir, "group_tfr_AllEEG_pain_baseline_logratio.png", logger=logger)

        fig = grp_n.plot(picks="AllEEG", show=False)
        try:
            fig.suptitle(f"Group TFR: All EEG — Non-pain (baseline logratio, n={n_subj})", fontsize=12)
        except Exception:
            pass
        _save_fig(fig, out_dir, "group_tfr_AllEEG_nonpain_baseline_logratio.png", logger=logger)

        fig = grp_d.plot(picks="AllEEG", show=False)
        try:
            fig.suptitle("Group TFR: All EEG — Pain minus Non (baseline logratio)", fontsize=12)
        except Exception:
            pass
        _save_fig(fig, out_dir, "group_tfr_AllEEG_pain_minus_non_baseline_logratio.png", logger=logger)
    except Exception as e:
        if logger:
            logger.warning(f"Group AllEEG pain/non plot failed: {e}")
    ...
    for band_name, (fmin, fmax) in bands.items():
        ...
        for col, (tmin_win, tmax_win) in enumerate(zip(window_starts, window_ends)):
            ...
            if diff_data is not None:
                # Cluster-based significance for this window and minimal cluster p
                sig_mask = None
                cluster_p_min = cluster_k = cluster_mass = None
                fdr_txt = ""
                if DIFF_ANNOTATION_ENABLED:
                ...
                try:
                    ...
                    # Add mean difference with cluster p if available
                    mu = float(np.nanmean(diff_data))
                    pct = (10.0 ** (mu) - 1.0) * 100.0
                    cl_txt = (_format_cluster_ann(cluster_p_min, cluster_k, cluster_mass) or fdr_txt) if DIFF_ANNOTATION_ENABLED else ""
                    if cl_txt:
                        axes[2, col].text(0.5, 1.08, f"Δ%={pct:+.1f}% | {cl_txt}", transform=axes[2, col].transAxes, ha="center", va="bottom", fontsize=8)
                    else:
...
def main_group(
    ...
    logger = get_group_logger("time_frequency_analysis", LOG_FILE_NAME)
    ...
    if temperature_strategy in ("pooled", "both"):
        ...
        # Group temporal topomaps across windows (max vs min temperature)
        try:
            ...
            logger.warning(f"Group temporal topomaps (max/min temp) failed: {e}")
    ...
    # Group All-EEG scalp-mean pain vs non and difference
    try:
        ...
        logger.warning(f"Group AllEEG pain/non plots failed: {e}")
...
def main(subject: str = "001", task: str = DEFAULT_TASK,
    ...
    logger = get_subject_logger("time_frequency_analysis", subject, LOG_FILE_NAME)
    ...
    # Baseline QC before applying baseline corrections/contrasts
    _log_baseline_qc(power, BASELINE, logger)
    ...
    # Subject-level HTML report via mne.Report removed per user request
